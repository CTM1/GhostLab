...
public class RecursiveMaze implements LabyrInterface {
  ...
  private void recursiveChamber(boolean grid[][], int startX, int startY, int endX, int endY) {
    ...
    if (chamberSizeX == 2 && chamberSizeY == 2) return;
    if (chamberSizeX < 2 || chamberSizeY < 2) {
      // System.out.println(String.format("Aborting (%d, %d) to (%d, %d)", startX, startY, endX,
      // endY));
      return;
    }
    ...
    boolean horizontal;
    if (chamberSizeX < chamberSizeY) horizontal = true;
    else if (chamberSizeY < chamberSizeX) horizontal = false;
    else horizontal = rand.nextInt(3) + 1 == 1;
    ...
    if (horizontal) {
      ...
      while (!wallselected) {
        if (tries > 100 * chamberSizeY) {
          // System.out.println(String.format("Couldn't find hwall for (%d, %d) to (%d, %d)",
          // startX, startY, endX, endY));
          return;
        }

        wallY = rand.nextInt(endY - startY) + startY + 1;
        if (grid[wallY][startX - 1] && grid[wallY][endX + 1]) wallselected = true;
        tries++;
      }

      int holeX = rand.nextInt(endX + 1 - startX) + startX;
      // System.out.println("Hole at x="+holeX);
      for (int i = 0; i < chamberSizeX; i++) grid[wallY][startX + i] = (startX + i != holeX);
      ...
      while (!wallselected) {
        if (tries > 100 * chamberSizeX) {
          // System.out.println(String.format("Couldn't find vwall for (%d, %d) to (%d, %d)",
          // startX, startY, endX, endY));
          return;
        }
        wallX = rand.nextInt(endX - startX) + startX;
        if (grid[startY - 1][wallX] && grid[endY + 1][wallX]) wallselected = true;
        tries++;
      }
      int holeY = rand.nextInt(endY + 1 - startY) + startY;
      // System.out.println("Hole at y="+holeY);
      for (int i = 0; i < chamberSizeY; i++) grid[startY + i][wallX] = (startY + i != holeY);
  ...
  public RecursiveMaze(int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (y == 0 || y == height - 1 || x == 0 || x == width - 1) grid[y][x] = true;
      }
    }
  ...
  public int[] emptyPlace() {
    ...
    while (tries < (width * height) * 2) {
      ...
      if (!grid[y][x]) return new int[] {x, y};
      tries++;
    }
    return null;
  }
  ...
  public int tryMove(int x, int y, int direction, int distance) {
    ...
    for (int i = 0; i < distance; i++) {
      if (grid[npY + wy][npX + wx]) return i;
      npX += wx;
      npY += wy;
    }
